...
export function useKPI3Data() {
  ...
  const kpiData = useMemo(() => {
    if (apiData && apiData["MA-KPI-3"]) {
      return { ...apiData["MA-KPI-3"], dataSource: "api" as const };
    }
    const filtered = getFilteredQuarterlyValue(maKPIData.kpi3);
    return {
