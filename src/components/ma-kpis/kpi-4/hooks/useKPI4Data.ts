...
export function useKPI4Data() {
  ...
  const kpiData = useMemo(() => {
    if (apiData && apiData["MA-KPI-4"]) {
      return { ...apiData["MA-KPI-4"], dataSource: "api" as const };
    }
    const filtered = getFilteredQuarterlyValue(maKPIData.kpi4);
    return {
