...
// Effective-decision rollout explicitly authorized on 2026-09-15. Backs up the complete report table before any UPDATE.
// Usage: node scripts/apply-ma-effective-decision.js --apply <absolute Downloads directory>
const fs = require('fs');
...
const artifactDir = path.resolve(__dirname, '../analysis/ma-effective-decision');
...
async function main() {
  ...
  if (process.argv[2] !== '--apply' || !downloads || !path.isAbsolute(downloads) || !fs.statSync(downloads).isDirectory()) {
    throw new Error('Usage: node scripts/apply-ma-effective-decision.js --apply <existing absolute Downloads directory>');
  }
  const reports = JSON.parse(fs.readFileSync(path.join(artifactDir, 'proposed-reports.json'), 'utf8'));
  const validation = JSON.parse(fs.readFileSync(path.join(artifactDir, 'candidate-2025.json'), 'utf8'));
  if (validation.checks.length !== 10 || validation.checks.reduce((n,g) => n + g.checks.length, 0) !== 350) throw new Error('Complete candidate reconciliation required');
  if (reports.length !== 59 || new Set(reports.map(r => r.id)).size !== 59) throw new Error('Expected 59 distinct approved reports');
  ...
  try {
    ...
    console.log('Committed 59 report updates; unrelated rows and fields unchanged.');
  } catch (error) {
  ...
  try {
    ...
    if (stored.length !== 59 || reports.some(p => !stored.some(r => r.id === p.id && r.query === p.query && r.filter_columns === p.filter_columns))) throw new Error('Fresh-connection persistence check failed');
    receipt.persistedReportsVerified = stored.length;
    await verify.query('ROLLBACK');
    console.log('Fresh connection confirms all 59 migrated reports are persisted.');
  } finally {
    await verify.end();
    fs.writeFileSync(path.join(backupDir, 'migration-receipt.json'), JSON.stringify(receipt, null, 2));
    fs.writeFileSync(path.join(artifactDir, 'migration-receipt.json'), JSON.stringify(receipt, null, 2));
