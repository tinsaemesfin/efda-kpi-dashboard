         ...
         SELECT bmd.id,
            ...
            COALESCE(
                CASE
                    ...
                    ELSE (0)::double precision
                END) AS assessor_assignment_days,
            COALESCE(((
                CASE
                    ...
                    ELSE (0)::double precision
                END + COALESCE(at.firr_to_stl_days, (0)::double precision)) + COALESCE(at.rtas_to_stl_days, (0)::double precision))) AS assessment_days,
            COALESCE((((((((COALESCE(at.stl_to_fir_days, (0)::double precision) + COALESCE(at.stl_to_sfa_days, (0)::double precision)) + COALESCE(at.stl_to_sfr_days, (0)::double precision)) + COALESCE(at.rtl_to_fir_days, (0)::double precision)) + COALESCE(at.rtl_to_sfa_days, (0)::double precision)) + COALESCE(at.rtl_to_sfr_days, (0)::double precision)) + COALESCE(at.stl_to_rtas_days, (0)::double precision)) + COALESCE(at.rtl_to_rtas_days, (0)::double precision))) AS teamleader_decision_days,
            COALESCE(at.fir_to_firr_days, (0)::double precision) AS applicant_response_fir_days,
            COALESCE(((((((((COALESCE(at.sfa_to_apr_days, (0)::double precision) + COALESCE(at.sfa_to_rtl_days, (0)::double precision)) + COALESCE(at.sfa_to_fir_days, (0)::double precision)) + COALESCE(at.sfr_to_rej_days, (0)::double precision)) + COALESCE(at.sfr_to_rtl_days, (0)::double precision)) + COALESCE(at.sfr_to_fir_days, (0)::double precision)) + COALESCE(at.apr_to_void_days, (0)::double precision)) + COALESCE(at.rej_to_void_days, (0)::double precision)) + COALESCE(at.void_to_rtl_days, (0)::double precision))) AS leo_final_decision_days
 ...
 SELECT 0 AS id,
    ...
    'Submodule Type Aggregation'::text AS aggregation_type,
  ...
  GROUP BY processing_times_with_totals.submodule_type_code
UNION ALL
 SELECT 0 AS id,
    true AS is_active,
    newid() AS rowguid,
    now() AS created_date,
    now() AS modified_date,
    'Approval Pathway Aggregation'::text AS aggregation_type,
    processing_times_with_totals.submodule_type_code,
    ...
    round((avg(processing_times_with_totals.total_processing_time_days))::numeric, 2) AS avg_total_processing_time_days
   FROM processing_times_with_totals
  GROUP BY processing_times_with_totals.approval_pathway_code, processing_times_with_totals.submodule_type_code
UNION ALL
 SELECT 0 AS id,
    true AS is_active,
    newid() AS rowguid,
    now() AS created_date,
    now() AS modified_date,
    'Module Aggregation'::text AS aggregation_type,
    processing_times_with_totals.submodule_type_code,
    NULL::text AS approval_pathway_code,
    ...
    round((avg(processing_times_with_totals.total_processing_time_days))::numeric, 2) AS avg_total_processing_time_days
   FROM processing_times_with_totals
  GROUP BY processing_times_with_totals.module_code, processing_times_with_totals.submodule_type_code
  ORDER BY 6, 7, 9;