...
DO $alignment$
BEGIN
  UPDATE kpi.kpi SET query='/* MA face/drilldown alignment v1 */
...
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=194 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
...
) SELECT * FROM effective_report;' AND filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]';
...
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=195 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM records GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=155 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
) SELECT * FROM effective_report;' AND filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]';
...
SELECT category_name, category_value, module_code, target_days,
  ...
  ROUND(AVG(processing_time_in_day) FILTER (WHERE processing_time_in_day>=0)::numeric,2) AS avg_processing_days
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=160 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=161 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=162 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=163 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM base
WHERE decision_time_in_days >= 0;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=118 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM base
WHERE decision_time_in_days >= 0
) SELECT * FROM effective_report;' AND filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]';
...
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=119 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=120 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''MDCN''::text AS submoduletype_code,target_days,
  ...
  COUNT(*) AS total_count,
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=114 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT category_name,category_value,module_code,target_days,
  ...
  ROUND(AVG(processing_time_in_day) FILTER (WHERE processing_time_in_day>=0)::numeric,2) AS avg_processing_days
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=109 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''FD''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=196 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=197 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code, ''FD''::text AS submoduletype_code, target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND effective_target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM records GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=156 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=164 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=165 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=166 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=167 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
WHERE decision_time_in_days >= 0;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=96 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=97 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=98 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''FD''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=115 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=110 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''FD''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=198 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=199 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code, ''FD''::text AS submoduletype_code, target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND effective_target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM records GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=157 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=168 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=169 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=170 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=171 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
WHERE decision_time_in_days >= 0;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=99 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=100 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=101 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''FD''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=108 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=111 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''MD''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=200 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=201 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code, ''MD''::text AS submoduletype_code, target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND effective_target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM records GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=158 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=172 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=173 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=174 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=175 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
WHERE decision_time_in_days >= 0;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=102 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=103 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=104 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''MD''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=116 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=112 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''CO''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=202 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Submission date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"submission_date"}]', modified_date=NOW()
  WHERE id=203 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code, ''CO''::text AS submoduletype_code, target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND effective_target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM records GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=159 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=176 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=177 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"FieldName":"dateFilter","DType":{"id":"6","name":"DateRange"},"IsInnerFilter":true,"Title":"Decision date","Alias":"","OverridingFieldName":"decision_date","ParameterName":"dateFilter","Type":"DateRange"}]', modified_date=NOW()
  WHERE id=178 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
WHERE decision_time_in_days >= 0;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=105 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=106 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
ORDER BY category_name, category_value;
', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=107 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
SELECT module_code,''CO''::text AS submoduletype_code,target_days,
  ...
  ROUND((COUNT(*) FILTER (WHERE processing_time_in_day BETWEEN 0 AND target_days)*100.0/NULLIF(COUNT(*),0))::numeric,2) AS percentage
FROM base GROUP BY module_code,target_days ORDER BY module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=117 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
...
FROM categorized GROUP BY category_name,category_value,module_code,target_days
ORDER BY category_name,category_value,module_code;', filter_columns='[{"Type":"DateRange","Alias":"","DType":{"id":"6","name":"DateRange"},"Title":"Decision date","FieldName":"dateFilter","IsInnerFilter":true,"ParameterName":"dateFilter","OverridingFieldName":"decision_date"}]', modified_date=NOW()
  WHERE id=113 AND query='/* MA effective decision v2: agreed status-log dates; current outcomes retained. */
WITH effective_status_log AS MATERIALIZED (
 SELECT ma_id,modified_date,from_status_code,to_status_code FROM license.vwma_log_status_new
 WHERE to_status_code IN (''APR'',''REJ'',''SUSP'',''CNCL'')
), effective_per_ma AS (
